...
class VoucherController extends Controller
{
    ...
    public function modifyGet(Request $request, $id)
    {
    ...
    public function modifyIncomePost(Request $request)
    {
        ...
        for($i = 0; $i < 10; $i++)
        {
            ...
            // Generar Voucherdetail
            if(isset($voucher->voucherdetails[$i]))
            {
                $voucher->voucherdetails[$i]->voucher_id = $voucher->id;
                $voucher->voucherdetails[$i]->account_id = $account->id;
                if($account->ctacte2 == 'S')
                    $voucher->voucherdetails[$i]->identification_id = $identification->id;
                    $voucher->voucherdetails[$i]->detail = $detalles[$i];
                if($doctype_ids[$i])
                    $voucher->voucherdetails[$i]->doctype_id = $doctype_ids[$i];
                if($doc_numbers[$i])
                    $voucher->voucherdetails[$i]->doc_number = $doc_numbers[$i];
                $doctype_ids[] = $request->input('doctype_id'.$i);
                $doc_numbers[] = $request->input('doc_number'.$i);
    
                $voucher->voucherdetails[$i]->date = Carbon::createFromDate(intval(explode('-', $fechas[$i])[2]),intval(explode('-', $fechas[$i])[1]),intval(explode('-', $fechas[$i])[0]));
                $voucher->voucherdetails[$i]->debit = $deberes[$i];
                $voucher->voucherdetails[$i]->credit = $haberes[$i];
    
                $voucher->voucherdetails[$i]->save();
            }
            else
            {
                $voucherdetail = new Voucherdetail();
        ...
        $this->addlog('Modificó voucher de Ingreso: ' . $voucher->sequence . '-' . $voucher->date->year);
        $request->session()->flash('success', 'El voucher ha sido modificado exitosamente');
        return redirect('/view/voucher/I/' . $voucher->sequence . '-' . $voucher->date->year);
    }
    ...
    public function modifyOutcomePost(Request $request)
    {
        ...
        $this->addlog('Modificó voucher de Egreso: ' . $voucher->sequence . '-' . $voucher->date->year);

        $request->session()->flash('success', 'El voucher ha sido modificado exitosamente');
        return redirect('/view/voucher/E/' . $voucher->sequence . '-' . $voucher->date->year);
    }
    ...
    public function modifyTransferPost(Request $request)
    {
        ...
        $this->addlog('Modificó voucher de Traspaso: ' . $voucher->sequence . '-' . $voucher->date->year);

        $request->session()->flash('success', 'El voucher ha sido modificado exitosamente');
        return redirect('/view/voucher/T/' . $voucher->sequence . '-' . $voucher->date->year);
    }
